  ...
  async listGuilds(): Promise<GuildsResponse> {
    const guilds = await Promise.all(
      [...this.client.guilds.cache.values()].map(async (guild) => {
        ...
        let channels: Awaited<ReturnType<typeof guild.channels.fetch>>;
        try {
          channels = await guild.channels.fetch();
        } catch (err) {
          if (err instanceof DiscordAPIError) {
            this.logger.warn(
              `Failed to fetch channels for guild ${guild.id}: ${err.message}`,
            );
            return { channels: [], id: guild.id, name: guild.name };
          }
          throw err;
        }
        ...
        return { channels: sendable, id: guild.id, name: guild.name };
      }),
    );
